...
use crate::{omnijot_file_dir, tauri_cmd};
...
tauri_cmd!{
    pub fn get_omnijot_file_meta(file_name: String) -> anyhow::Result<OmniJotFileMeta> {
        let resolved_name = resolve_ojf_file_name(&file_name).context("校验笔记文件名")?;
        let path = omnijot_file_dir::get_omnijot_file_dir(resolved_name.clone());
        if !path.is_file() {
            return Err(anyhow::anyhow!("文件不存在或者是个目录"));
        }
        let compressed = is_ojf_compressed(&path).context("检测笔记压缩格式")?;
        ...
        deserialize_omnijot_file_meta_xml(&xml).context("解码笔记元信息")
    }

    pub fn get_omnijot_file_body(file_name: String) -> anyhow::Result<OmniJotFileBody> {
        let resolved_name = resolve_ojf_file_name(&file_name).context("校验笔记文件名")?;
        let path = omnijot_file_dir::get_omnijot_file_dir(resolved_name.clone());
        if !path.is_file() {
            return Err(anyhow::anyhow!("文件不存在或者是个目录"));
        ...
        let compressed = is_ojf_compressed(&path).context("检测笔记压缩格式")?;
        let mut cache = OmniJotFileCache::new_named(&resolved_name)
        ...
        let body = deserialize_omnijot_file_body_xml(body_xml).context("解码笔记正文")?;
        info!("读取笔记正文成功：{}", resolved_name);
        Ok(body)
    }

    pub fn set_omnijot_file_body(file_name: String, content: String) -> anyhow::Result<()> {
        let resolved_name = resolve_ojf_file_name(&file_name).context("校验笔记文件名")?;
        let path = omnijot_file_dir::get_omnijot_file_dir(resolved_name.clone());
        if !path.is_file() {
            return Err(anyhow::anyhow!("文件不存在或者是个目录"));
        ...
        let body_xml = serialize_omnijot_file_body_xml(OmniJotFileBody { content })
            .context("编码笔记正文")?;
    ...
    pub fn create_omnijot_file(
        ...
        let resolved_name = resolve_ojf_file_name(&file_name).context("校验笔记文件名")?;
        let cache = OmniJotFileCache::new().context("创建笔记缓存")?;
        let meta_xml = serialize_omnijot_file_meta_xml(omnijot_file_info)
            .context("编码笔记元信息")?;
        ...
        let file_path = omnijot_file_dir::get_omnijot_file_dir(resolved_name.clone());
        // 前端已拦截同名，后端仍须防覆盖既有笔记
        if file_path.exists() {
...
fn resolve_ojf_file_name(file_name: &str) -> anyhow::Result<String> {
